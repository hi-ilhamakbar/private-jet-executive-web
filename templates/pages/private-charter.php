...
<section class="section section--pearl">
    <div class="container form-shell">
        ...
        <form class="inquiry-form inquiry-form--light" action="/private-charter" method="post" data-inquiry-form>
            ...
            <div class="form-grid">
                <div class="field"><label for="departure">Departure <span aria-hidden="true">*</span></label><input id="departure" name="departure" autocomplete="off" placeholder="City or airport" required value="<?= $escape($old['departure'] ?? '') ?>"><?php if (isset($errors['departure'])): ?><small class="field-error"><?= $escape($errors['departure']) ?></small><?php endif; ?></div>
                <div class="field"><label for="arrival">Arrival <span aria-hidden="true">*</span></label><input id="arrival" name="arrival" autocomplete="off" placeholder="City or airport" required value="<?= $escape($old['arrival'] ?? '') ?>"><?php if (isset($errors['arrival'])): ?><small class="field-error"><?= $escape($errors['arrival']) ?></small><?php endif; ?></div>
                ...
                <div class="field"><label for="departure_time">Estimated departure time <span aria-hidden="true">*</span></label><input id="departure_time" name="departure_time" type="time" required value="<?= $escape($old['departure_time'] ?? '') ?>"><?php if (isset($errors['departure_time'])): ?><small class="field-error"><?= $escape($errors['departure_time']) ?></small><?php endif; ?></div>
                <div class="field" data-return-field <?= ($old['journey_type'] ?? '') !== 'return' ? 'hidden' : '' ?>><label for="return_date">Return date <span aria-hidden="true">*</span></label><input id="return_date" name="return_date" type="date" min="<?= $today ?>" value="<?= $escape($old['return_date'] ?? '') ?>"><?php if (isset($errors['return_date'])): ?><small class="field-error"><?= $escape($errors['return_date']) ?></small><?php endif; ?></div>
                <div class="field" data-return-field <?= ($old['journey_type'] ?? '') !== 'return' ? 'hidden' : '' ?>><label for="return_time">Estimated return time <span aria-hidden="true">*</span></label><input id="return_time" name="return_time" type="time" value="<?= $escape($old['return_time'] ?? '') ?>"><?php if (isset($errors['return_time'])): ?><small class="field-error"><?= $escape($errors['return_time']) ?></small><?php endif; ?></div>
                ...
                <div class="field field--phone"><label for="charter_country_code">Contact number <span aria-hidden="true">*</span></label><div class="phone-group"><select id="charter_country_code" name="country_code" required aria-label="Country calling code"><option value="">Country code</option><?php $selectedCountry = $old['country_code'] ?? ''; require dirname(__DIR__) . '/partials/country-options.php'; ?></select><input id="charter_phone" name="phone" type="tel" inputmode="numeric" pattern="[0-9]*" autocomplete="tel-national" placeholder="Phone number" required value="<?= $escape($old['phone'] ?? '') ?>"></div><?php if (isset($errors['country_code']) || isset($errors['phone'])): ?><small class="field-error"><?= $escape($errors['country_code'] ?? $errors['phone']) ?></small><?php endif; ?></div>
