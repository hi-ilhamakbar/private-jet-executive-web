    ...
    <div class="container form-shell">
        ...
        <form class="inquiry-form" action="/contact" method="post" data-inquiry-form>
            ...
            <div class="form-grid">
                ...
                <div class="field field--phone"><label for="country_code">Contact number <span aria-hidden="true">*</span></label><div class="phone-group"><select id="country_code" name="country_code" required aria-label="Country calling code" data-country-picker><option value="">Country code</option><?php $selectedCountry = $old['country_code'] ?? ''; require dirname(__DIR__) . '/partials/country-options.php'; ?></select><input id="phone" name="phone" type="tel" inputmode="numeric" pattern="[0-9]*" autocomplete="tel-national" placeholder="Phone number" required value="<?= $escape($old['phone'] ?? '') ?>"></div><?php if (isset($errors['country_code']) || isset($errors['phone'])): ?><small class="field-error"><?= $escape($errors['country_code'] ?? $errors['phone']) ?></small><?php endif; ?></div>
...
<section class="section section--charcoal"><div class="container centered-copy"><p class="eyebrow">Personal service</p><h2>Based in Semarang, connected worldwide.</h2><p>Our team is available by email for charter and travel requirements.</p></div></section>
